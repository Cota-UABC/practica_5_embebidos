...
esp_err_t device_read_byte(uint8_t *data)
{
	...
	sendbuf[0] = 5;//code to receive data
	...
	spi_transaction.length = 1 * 8;
	spi_transaction.rxlength = 1 * 8;
...
void app_main(void)
{
    ...
    while (1)
    {
    	...
    	ESP_ERROR_CHECK(device_write_byte(value));
    	ESP_LOGI(TAG, "Data send =  %d", value);

    	vTaskDelay(pdMS_TO_TICKS(300));

    	ESP_ERROR_CHECK(device_read_byte(data));
    	ESP_LOGI(TAG, "Data received =  %d", data[0]);
